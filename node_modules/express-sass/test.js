var express = require('express'),
	...
	path = require('path'),
	port = 8080,
	pathArr = [__dirname+"/wwwroot",__dirname+"/public"],
	routerObj = {}
...
pathArr.map(function(val,index){
	var router = express.Router()
	//router.use(express.static(__dirname + val))
	manager(router,val)
	routerObj[val] = router
...
function portIsOccupied (port) {
	...
	server.on('listening', function () { // 执行这块代码说明端口未被占用
		...
		app.use(function(req,res,next){
			routerObj[pathArr[1]](req,res,next)
		})
...
function manager(router,src){
		router.get('**/*.shtml',function(req,res,next){  // .shtml 后缀文件 修改header 为html
			var filename = src + req.url
			toReload(filename,src,function(html){
				res.send(html)
			})
		})
		 
		router.get('*.html',function(req,res,next){ // .html 后缀文件如不存在映射到相同文件名的 .shtml文件
			var url = req.url,
		 	...
		 	fs.exists(file_url,function(exists){
		 		if(exists){
		 			toReload(base + ".html",src,function(html){
						res.send(html)
					})
		 		}else{
		 			fs.exists(base + ".shtml",function(exists){
		 				if(exists){
		 					toReload(base + ".shtml",src,function(html){
								res.send(html)
							})
		 				}else{
		 					res.send("没有找到这个文件哦！！")
		...
		router.get('*.css',function(req,res){  // 将.css 文件映射到 .scss上
			var url = req.url;
			...
			var	filename = path.basename(url,'.css'),
		 	...
		 	router.use(express.static(src,{ // 静态托管
		...
		router.use(serveIndex(src,{
			'icons': true
		}))
}


function toReload(filename,src,callback){  // 处理html和shtml文件，加入livereload.js并处理ssi语法
